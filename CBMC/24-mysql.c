...
//  clang -emit-llvm -S -o 24-mysql.ll 24-mysql.c
// nidhugg --unroll=10 --transform=24-mysql.trans.ll 24-mysql.ll
//  time nidhugg -arm 24-mysql.ll
#include <assert.h>
#include <stdatomic.h>
#include <stdio.h>
#include <stdbool.h>
...
//    return atomic_compare_exchange_strong(&m_lock_word, &expected, MUTEX_STATE_LOCKED);
    if (m_lock_word == expected) {
        m_lock_word = MUTEX_STATE_LOCKED;  // 尝试获取锁
        return true;  // 返回 true 表示成功获取锁
    }
    return false;  // 返回 false 表示锁已被其他线程获取
}
...
//    return atomic_exchange(lock_word, new_val);
    int old_val = *lock_word;  // 保存当前值
    *lock_word = new_val;      // 设置新值
    return old_val;            // 返回旧值
}

...
// cbmc --arch arm64 --mm pso  4-MySQL1.c
#include <stdbool.h>
#include <pthread.h>
#include <stdio.h>
#include <assert.h>
...
/*=====================================*/
        ...
        bool		recursive)	/*!< in: true if recursion
					allowed */
{
    int	curr_thread	= os_thread_get_curr_id();
...
/*===============*/
        ...
        int		pass	/*!< in: pass value; != 0, if the lock will
				be passed to another thread to unlock */)	/*!< in: line where requested */
{
...
void* thread_a_func(void* arg) {
    ...
    if (rw_lock_x_lock_low(&my_lock, 0)) {

...
//  clang -emit-llvm -S -o 22-openmpi.ll 22-openmpi.c
//  nidhugg --unroll=10 --transform=22-openmpi.trans.ll 22-openmpi.ll
//  time nidhugg -arm 22-openmpi.trans.ll
#include <stdatomic.h>
#include <stdio.h>
#include <stdlib.h>
#include <stdio.h>
...
//            item->item_free = 0;
//           // printf("1");
//            assert(item ->item_free == 0);
//            return next;
//        }
        if (lifo->opal_lifo_head.item == next) {
            opal_atomic_wmb();
            /* now safe to pop this item */
            item->item_free = 0;
            // printf("1");
...
//            continue;
//        }
        if (item->item_free != 0) { // 检查item_free，非原子检查
            continue;
        }
        item->item_free = 1;
        //printf("3");
...
//            break;
//        }
        if (lifo->opal_lifo_head.item == item) {
            lifo->opal_lifo_head.item = item->opal_list_next; // 更新头部指针
            break;
        }
        item ->item_free = 0;
    }
